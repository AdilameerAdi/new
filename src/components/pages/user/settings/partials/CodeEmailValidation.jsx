...
export const CodeEmailValidation = ({ email, setEmail }) => {
  ...
  const reSendCode = async () => {
    ...
    const formData = { Email: account?.email, email  }
  ...
  const submit = async (data) => {
    ...
    const url = `${apiConfig().endpoint}/auth/update/email/validation`
  ...
  return (
    <form className="flex flex-col gap-4 justify-center items-center animate__animated animate__fadeInRight" onSubmit={handleSubmit(submit)}>
      ...
      <p className="text-sm text-center">
        {t("We have sent a verification code to your email address")} <br /><b>{email}</b>, {t("please enter it here.")}
      </p>
      <Input
          icon={<Lock />}
          id="code"
