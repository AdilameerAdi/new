...
export const DeleteClaims = ({ modal, setModal, claims, setClaims }) => {

  const dispatch = useDispatch()

  const submit = async () => {

      const url = `${apiConfig().endpoint}/ranking/prizes/remove`
  
      dispatch(setLoad(false));
  
      await axios.post(url, { claims }, apiConfig().axios)
        .then(res => {
          Swal.fire({
            toast: true,
    ...
    >
      <div className="flex flex-col gap-4">
        ...
        <h5 className="text-xl text-center">
          Are you sure you want to delete {claims.length} claims?
        </h5>          
      </div>
