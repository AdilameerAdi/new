...
export const DeleteCoupons = ({ modal, setModal, coupons, setCoupons }) => {

  const dispatch = useDispatch()

  const handleDelete = async () => {

    const url = `${apiConfig().endpoint}/coupons/remove/`

    dispatch(setLoad(false));

    await axios.post(url, { coupons }, apiConfig().axios)
      .then(res => {
        Swal.fire({
          toast: true,
    ...
    >
      <div className="flex flex-col gap-4">
        ...
        <h5 className="text-xl text-center">
          Are you sure you want to delete {coupons.length} coupons?
        </h5>          
      </div>
